...
const userController = {
  ...
  register: async (request, response) => {
    ...
    try {
      ...
      if (request.body.password !== request.body.passwordConfirm) {
        return response.send({
          formData: request.body,
          error: 'La confirmation de votre mot de passe a échoué',
        });
      }
  ...
  login: async (request, response) => {
    ...
    try {
      ...
      return response.send({
        user: removePassword(user.dataValues),
        logged: true,
      });
    } catch (error) {
      response.status(500).send(error);
  ...
  updatePassword: async (request, response) => {
    try {
      ...
      if (request.body.newPassword !== request.body.newPasswordConfirm) {
        return response.send({
          formData: request.body,
          error: 'La confirmation de votre nouveau mot de passe a échoué',
        });
      }

      // encryptage du password
      const encryptedPassword = await bcrypt.hash(request.body.newPassword, 10);
      ...
      response.send({
        user: removePassword(updatedUserPassword.dataValues),
  ...
  resetPassword: async (request, response) => {
    try {
      ...
      response.send({
        user: removePassword(updatedUserPassword.dataValues),
        message:
          'Votre mot de passe a bien été réinitialisé, veuillez vous connecter',
      });
    } catch (error) {
      response.status(500).send(error);
    }
